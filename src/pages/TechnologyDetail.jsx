...
function TechnologyDetail() {
  ...
  const updateStatus = (newStatus) => {
    ...
    if (saved) {
      ...
      const updated = technologies.map((tech) =>
        tech.id === parseInt(techId, 10) ? { ...tech, status: newStatus } : tech
      );
  ...
  const deleteTechnology = () => {
    ...
    if (saved) {
      const technologies = JSON.parse(saved);
      const updated = technologies.filter((tech) => tech.id !== parseInt(techId, 10));
      localStorage.setItem('technologies', JSON.stringify(updated));
    ...
    <div className="page">
      ...
      <div className="technology-detail">
        ...
        <div className="detail-section danger">
          <h3>Удаление технологии</h3>
          <button type="button" className="btn btn-danger" onClick={deleteTechnology}>
            Удалить технологию
          </button>
        </div>
      </div>
